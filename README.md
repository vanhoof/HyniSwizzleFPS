...
Designed to be injected alongside [HyniSign](https://github.com/vanhoof/HyniSign) (Microsoft sign-in fix) and [HynisLoader](https://github.com/congcq/HynisLoader)'s `libhynisloader.dylib` (RenderDragon shader loader). Each is a separate dylib injection — no shared state.
...
4. Under **Inject dylibs/deb/bundle**, add `build/HyniSwizzleFPS.dylib`. (You can add other dylibs in the same step — `libhynisloader.dylib` from HynisLoader, `HyniSign.dylib`, `MCClient.dylib`.)
...
- [HynisLoader](https://github.com/congcq/HynisLoader) by congcq — the upstream RenderDragon shader-loading dylib (`libhynisloader.dylib`) this is meant to coexist with.
